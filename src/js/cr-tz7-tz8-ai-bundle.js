...
(function applyTZ8RealAIDecisionEngine(){
  ...
  function buildRealDecision(routes){
    ...
    function uaeSalikMattersAcrossRoutesTz8(rlist) {
      const lst = (Array.isArray(rlist) ? rlist : []).filter(Boolean);
      if (!lst.length) return false;
      const costs = lst.map(function(r) { return salikAed(r); });
      let idx;
      for (idx = 0; idx < costs.length; idx++) {
        if (costs[idx] >= 0.01) return true;
      }
      const mn = Math.min.apply(null, costs);
      const mx = Math.max.apply(null, costs);
      return mx - mn >= 0.01;
    }
    const salikMattersGlobal = uaeSalikMattersAcrossRoutesTz8(routesForRanking);
    const d = dict();
    const p = pref();
    const f = filters();
    ...
    if (second && salikMattersGlobal && salikAed(second) >= 0.01 && salikAed(best) < 0.01) why.push(d.salikFreeVs);
    if (second) {
    ...
    if (best && best.uaeDestAbuDhabi) {
      ...
      if (salikMattersGlobal && sa > 0.01) trade.push(tpl(d.salikDubaiSegmentNarr, { aed: Math.round(sa), gates: sg }));
    } else {
      if (best && salikMattersGlobal && sa > 0.01) trade.push(tpl(d.salikCostNarr, { aed: Math.round(sa), gates: sg }));
      else if (best && da > 0.01) trade.push(tpl(d.darbCostNarr, { aed: Math.round(da), gates: dg }));
    ...
    let whenText = salikMattersGlobal ? d.noWait : d.noWaitTraffic;
    if (type === 'SIMILAR' && salikMattersGlobal && second && Math.abs(salikAed(best) - salikAed(second)) > 1.5) {
