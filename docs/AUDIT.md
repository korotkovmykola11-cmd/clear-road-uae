...
- Источник логики: **`input/index.html`** (CSS, i18n, **TZ8 RTL**, **TZ9 voice**, **UX self-check**, **UX diag bootstrap**, **TZ1+TZ2 patch**, **TZ3 i18n clean**, **cr-empty-state**, **ключ Maps**) + `src/styles/*.css` + `src/i18n/*.js` + `src/js/*.js`. Ключ при сборке: env или `src/secrets/maps-api-key.txt` (не в git). Для CI/прод: **`npm run build:prod`** (без ключа сборка падает).
...
| **3** | Срез IIFE/дубли | **Сделано:** empty-state, TZ9 voice, UX self-check, TZ8 RTL, UX diag bootstrap, **TZ1+TZ2 final patch** (`cr-tz1-tz2-final-patch`), **TZ3 i18n clean** (`cr-tz3-i18n-clean-layer`) — в `src/js` + плейсхолдеры в `input`. Дальше по желанию: TZ4/TZ6/TZ10 и др. отдельные `<script>` без трогания ядра маршрута. |
