...
- Источник логики: **`input/index.html`** + `src/js/*` (в т.ч. TZ4, TZ6, TZ6B, TZ7+TZ8 bundle, … см. комментарий в `scripts/build.mjs`). Ключ при сборке: env или `src/secrets/maps-api-key.txt` (не в git). Для CI/прод: **`npm run build:prod`** (без ключа сборка падает).
...
| **3** | Срез IIFE/дубли | **Сделано:** empty-state, TZ9 voice, UX self-check, TZ8 RTL (отдельный слой), UX diag bootstrap, TZ1+TZ2 patch, TZ3 i18n, **TZ4 mobile guard**, **TZ6 AI clean**, **TZ6B final**, **TZ7+TZ8** (`cr-tz7-tz8-ai-bundle` — один тег с двумя IIFE подряд, как в монолите). Плейсхолдеры в `input`. Дальше: только основной большой `<script>` и хвостовые вставки. |
